...
//			fprintf(stderr, "%d\n", vlist.size());
    ...
    std::optional<block> get_block_at(int x, int y, int z) {
        if (x >= 0 && x < W && y >= 0 && y < H && z >= 0 && z < W)
            return data[y][x][z];
        return {};
    }

    std::optional<std::reference_wrapper<block>> get_block_ref_at(int x, int y, int z,
                                                                  bool taint = true) {
        if (x >= 0 && x < W && y >= 0 && y < H && z >= 0 && z < W) {
            dirty |= taint;
            return data[y][x][z];
...
private:
    bool face_occluded(glm::ivec3 position, world::block::block_face face) {
    ...
    world::block::block_face_set visible_faces(glm::ivec3 position) {
        using namespace world::block;
