...
BlockShader::BlockShader()
{
	...
	const auto total = static_cast<unsigned int>((3 + 1) * sizeof(GLint) + (3 * 4) * sizeof(GLfloat));
	this->MeshAttributes = std::vector<gl::VArrayAttribute>{
		...
		},
		{
			...
			static_cast<unsigned int>(4 * sizeof(GLint) + 3 * sizeof(GLfloat)), false, 0
		},
		{
			shader_->GetAttribute("texcoords") + 2, 3, gl::FLOAT, total,
			static_cast<unsigned int>(4 * sizeof(GLint) + 6 * sizeof(GLfloat)), false, 0
		},
		{
			shader_->GetAttribute("texcoords") + 3, 3, gl::FLOAT, total,
			static_cast<unsigned int>(4 * sizeof(GLint) + 9 * sizeof(GLfloat)), false, 0
		},
	};
