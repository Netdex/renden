    ...
    if (action == GLFW_PRESS) {
        switch (button) {
            case GLFW_MOUSE_BUTTON_LEFT:
                if (target)
                    cnk_mgr->get_block_ref_at(target->first)->get() = block();
                break;
            case GLFW_MOUSE_BUTTON_RIGHT:
                if (target)
                    cnk_mgr->get_block_ref_at(target->first + target->second, true)->get() = block(active_block);
                break;
        }
    }
}
...

    {
        ...
        for (int cz = 0; cz < scz; cz++) {
            for (int cx = 0; cx < scx; cx++) {
                ...
                for (int i = 0; i < 5; i++)
                    cnk->get_block_ref_at(8, i + 3 + 64, 8)->get() = block(17);
                for (int y = 0; y < 3 + 64; y++)
                    for (int z = 0; z < 16; z++)
                        for (int x = 0; x < 16; x++) {
                            if (y == 2 + 64)
                                cnk->get_block_ref_at(x, y, z)->get() = block(2);
                            else
                                cnk->get_block_ref_at(x, y, z)->get() = block(3);
                        }

                for (int y = 8 + 64; y < 13 + 64; y++) {
                    for (int z = 6; z < 11; z++)
                        for (int x = 6; x < 11; x++) {
                            cnk->get_block_ref_at(x, y, z)->get() = block(18);
                        }
                }
                cnk->get_block_ref_at(10, 10 + 64, 10)->get() = block(5);
                cnk->get_block_ref_at(10, 10 + 64, 9)->get() = block(4);
                cnk->get_block_ref_at(10, 10 + 64, 8)->get() = block(40);
            }
        }
